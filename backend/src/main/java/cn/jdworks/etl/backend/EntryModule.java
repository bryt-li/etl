 ...
 */
@IocBy(type = ComboIocProvider.class, args = { "*js", "ioc/", "*anno", "cn.jdworks.etl.backend", "*tx" })
/*
...
public class EntryModule {
	...
	public boolean heartbeat(String id) {
		try {
			this.executorManager.resetExecutorTimeoutTick(UUID.fromString(id));
			return true;
		} catch (Throwable t) {
			LOG.debug(t);
			return false;
	...
	public boolean shutdown(String id) {
		try {
			this.executorManager.shutdownExecutor(UUID.fromString(id));
			return true;
		} catch (Throwable t) {
			LOG.debug(t);
			return false;
